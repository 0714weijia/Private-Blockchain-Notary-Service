...
class Blockchain {
  ...
  addBlockWithBody(body, fun) {
    this.addBlock(new Block(body), fun);
  }

  // Add new block
  addBlock(newBlock, fun) {
    this.getBlockHeight().then((height) => {
      ...
      if (newBlock.height === 0) {
        ...
        db.put(newBlock.height, JSON.stringify(newBlock).toString(), function (err) {
          if (fun) fun(err, JSON.stringify(newBlock).toString());
  ...
  getBlockByHash(hash) {
    return new Promise(function (resolve, reject) {
      db.createReadStream()
        ...
        .on('close', function () {
          resolve("Fail to get block with hash " + hash);
        });
    });
  }
  ...
  getBlockByAddress(address) {
    return new Promise(function (resolve, reject) {
      ...
      db.createReadStream()
        .on('data', function (data) {
          let block = JSON.parse(data.value);
          if (block.body && block.body.address && address === block.body.address) {
            blocks.push(block);
          }
        })
        .on('error', function (err) {
        ...
        .on('close', function () {
          resolve(JSON.stringify(blocks).toString());
        });
    });
  }
